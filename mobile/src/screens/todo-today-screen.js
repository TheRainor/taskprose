...
export default function TodoTodayScreen({ navigation }) {
  const { tasks, loading, handleCheckbox, handleDelete } = useTaskManagement(navigation, "today");
  ...
  return (
    <View className="bg-indigo-950 min-h-screen">
      ...
      <View className="w-[87%] mt-[7%] self-center flex-1">
        <Text className="text-white text-2xl mb-4">Bugünkü Görevler</Text>

        <ScrollView>
          {tasks.length === 0 && (
            <Text className="text-gray-300 text-center mt-10">
              Bugün hiç görev eklemediniz.
            </Text>
          )}
          {tasks.map((task) => (
            <View
