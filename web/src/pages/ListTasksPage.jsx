...
export default function ListTasksPage() {
  ...
  const {
    ...
    handleToggle,
    handleDelete,
  } = useTasks(filter, navigate, listId);

  const { lists } = useLists(navigate);
  const currentList = lists.find((list) => list.id === parseInt(listId));

  const titleText = currentList
    ? t("lists.detail.title", { name: currentList.list_name })
